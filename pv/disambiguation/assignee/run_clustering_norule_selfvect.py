...
from pv.disambiguation.assignee.model import AssigneeModel_noconstraint
...
flags.DEFINE_string('assignee_name_model','resources/tfidf.pkl', '')
...
flags.DEFINE_string('run_id', 'run_5', 'data path')
...
def run_batch(canopy_list, outdir, loader, job_name='disambig'):
    ...
    encoding_model = AssigneeModel_noconstraint.from_flags(FLAGS)
...
if __name__ == "__main__":
    # load the assignee data set and create a list of doc
    doc = pd.read_excel('exp_out/assign_rev_v4.xlsx')
    doc_list = list(doc['assign'])
    ...
    re = tfidf2.fit(doc_list)


    with open('resources/tfidf.pkl', 'wb') as fout:
        pickle.dump(re, fout)
