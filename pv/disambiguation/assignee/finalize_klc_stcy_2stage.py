...
flags.DEFINE_string('input', '/kellogg/data/patents/output/assignee/run_33', '')
flags.DEFINE_string('assignee_name_mentions', '/kellogg/data/patents/output/assignee_mentions_2stage.records.pkl', '')
flags.DEFINE_string('path', '/kellogg/data/patents/patent_text/', '')
flags.DEFINE_string('output', '/kellogg/data/patents/output/disambiguation_2stage.tsv', '')
...
def main(argv):
    ...
    with open(FLAGS.output + 'assignee_disambiguation_stcy_2stage.pkl', 'wb') as fout:
        pickle.dump(mid2eid, fout)
    ...
    with open(FLAGS.feature_out + 'disambiguation_output_2stage.pkl', 'rb') as fin:
        final_output = pickle.load(fin)
    final_entity_id = []
    for i in range(len(final_output)):
        try:
            final_entity_id.append(mid2eid[final_output['mention_id'][i]])
        except:
            final_entity_id.append(missing_mid2eid[final_output['mention_id'][i]])
    final_output['entity_id_2stage'] = final_entity_id
    final_output.to_csv(FLAGS.feature_out + 'disambiguation_output_2stage.csv')
